...
async function StartServer() {
    ...
    // var DatabaseExists = await Database.CheckExistance();       
    //var thePost = await Post.getPost(5);
    //var thePost = new Post(5);
...
app.get('/viewpost', async (request, response) => {
    ...
    if (CurrentUser.UserType == "TCH") {
        ...
        UserOptions += "<p> اگر میخواهید فایلی بارگذاری شود آنرا انتخاب کنید</p>";
...
app.get('/db_init', async (request, res) => {

    var ClearContents = request.query.clearcontent;
    console.log("ClearContents = " + ClearContents);
    if (ClearContents) {

    }
    const Database = require("./Database")
    try {
        await Database.CreateDatabaseIfNot();
    }
    catch{ }
    try {
        console.log("Create Tables Starts here: ");
        await Database.CreateTables();
        console.log("Create Tables Ends here ");
    }
    catch{ }
...
app.post('/authenticate', async (request, response) => {
    ...
    if (username && password) {
        ...
        if (bcrypt.compareSync(password, CurrentUser.Password)) {
            let Message = "";
            let Title = "";
            const SiteLogs = require("./SiteLogs");
            const SiteLog = new SiteLogs(CurrentUser.UserName, moment(Date.now()).format('YYYY-MM-DD HH:mm:ss'));
            await SiteLog.Add();
